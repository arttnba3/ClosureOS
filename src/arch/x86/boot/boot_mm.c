...
static struct multiboot_tag_elf_sections *elf_info_tag = NULL;
...
static bool addr_is_in_used_range(phys_addr_t addr)
{
    ...
    for (shdr = (void*) &elf_info_tag->sections;
        ...
        /**
         * NOTE: we need to use the SAME logic here as how we allocate ELF mem
         * in the former code to keep the operation's consistance.
         */
...
static int boot_mm_pgtable_init(void)
{
    ...
    for (shdr = (void*) &elf_info_tag->sections;
        ...
        /**
         * ELF mapping for .text, .data, .rodata
         * 
         * NOTE: If there's no other global variables with initial values,
         * the type of `.data` segment would be turned into 0xe instead of 0x1
         * (if there're some global classes in the code),
         * which cause the whole section to be identified as "no data".
         * Therefore, we need to check for `SHT_SYMTAB` too here.
         */
        if ((shdr->sh_type & SHT_PROGBITS || shdr->sh_type & SHT_SYMTAB)
            && (shdr->sh_size != 0)) {
            /**
             * We set it starts from 1M in linker.lds, and this extra page
             * is the offset value of first section(boot) in ELF, as
             * the very begining page is for the ELF header, and we want
             * all the sections to be page-aligned.
             */
        ...
        /**
         * ELF mappiing for .bss
         * 
         * NOTE: If there's nothing in the .data segment,
         * (no global class && no global variables with values,
         * though it'll only occur in early developing stage, but I found it)
         * the length of it would be set to zero, which means that
         * there's nothing for it in the ELF. But in original logic we'll
         * at least map for one page, resulting in mis-aligned memory mapping.
         * Therefore, we make all the sections with 0-length to fall back here,
         * and allocating a single blank page to make them happy :)
         */
        else if (shdr->sh_type & SHT_NOBITS || shdr->sh_size == 0) {
            if (shdr->sh_size == 0) {
                shdr->sh_size = 1;  /* just let it allocate :) */
            }
    ...
    for (int i = 0; i < mmap_entry_nr; i++) {
        phys_addr_t base = mmap_tag->entries[i].addr & PAGE_MASK;
        phys_addr_t end = base + mmap_tag->entries[i].len;
    ...
    mmap_tag = (void*) phys_to_virt((phys_addr_t) mmap_tag);
    ...
    for (int i = 0; i < mmap_entry_nr; i++) {
        ...
        while (base < end) {
            ...
            case MULTIBOOT_MEMORY_AVAILABLE:
                ...
                pgdb_base[pfn].kc = NULL;
                pgdb_base[pfn].freelist = NULL;
                pgdb_base[pfn].obj_nr = 0;
